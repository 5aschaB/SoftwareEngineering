...
class Metric:
  def __init__(self, name, projectid, priority, score, trend, islow):
    self.name = name
...
## add     timeframe = numberOfTimeframes + 1   to metrics somewhere!
...
def recordSoftMetric(projectid, metric, scorelist):
    ...
    s_metric = SoftMetric(metric, projectid, priority, newscore, score_trend, score_low) ## some timeframe/metricid?
    return s_metric
...
def recordHardMetric(projectid, metric, username, repository):
    ...
    h_metric = HardMetric(metric, projectid, priority, newscore, score_trend, score_low) ## some timeframe/metricid?
    return h_metric
