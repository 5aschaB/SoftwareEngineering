...
class TestEmailConfirmationToken(BaseTestCase):
    ...
    def test_confirm_token_route_valid_token(self):
        ...
        with self.client:
            ...
            user = UserAccountTable.query.filter_by(
                email='unconfirmeduser@gmail.com').first_or_404()
            self.assertIsInstance(user.emailVerifiedTime, datetime.datetime)
            self.assertTrue(user.emailVerificationStatus)
    ...
    def test_confirm_token_route_expired_token(self):
        ...
        user = UserAccountTable(email='test@test1.com', password='test1', tokenGenerationTime=datetime.datetime.now(),
                                            emailVerificationStatus=False, emailVerifiedTime=None, roleType="Manager", is_admin=False)
        db.session.add(user)
        db.session.commit()
        token = generate_token('test@test1.com')
